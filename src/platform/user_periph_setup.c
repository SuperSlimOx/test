...
#endif
	RESERVE_GPIO(LED, GPIO_LED_PORT, GPIO_LED_PIN, PID_PWM2);
	RESERVE_GPIO(PUSH_BUTTON, GPIO_BUTTON_PORT, GPIO_BUTTON_PIN, PID_GPIO);   
	RESERVE_GPIO(EMITTER, GPIO_EMITTER_PORT, GPIO_EMITTER_PIN, PID_GPIO);	
...
#endif
	GPIO_ConfigurePin(GPIO_LED_PORT, GPIO_LED_PIN, OUTPUT, PID_PWM2, false);
	GPIO_ConfigurePin(GPIO_BUTTON_PORT, GPIO_BUTTON_PIN, INPUT_PULLUP, PID_GPIO, false ); // Push Button 
	GPIO_ConfigurePin(GPIO_EMITTER_PORT, GPIO_EMITTER_PIN, OUTPUT, PID_GPIO, true);	
...
void periph_init(void) 
{
   ...
   // Enable the pads
	...
	// LED를 PWM2로 사용하기 위한 TIMER2 초기설정
	led_init();
