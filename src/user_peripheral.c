...
{
#ifdef SLEEP_MODE_MODULE
	  // 게시 파라메터 업데이트 하는거 막음. (이거 살려놓으면 연결모드에서 게시모드 돌아올때
	  // 게시 파라메터 때문에 게시모드가 되기 전에 꺼지는 현상이 발생함, 어차피 파라메터 업데이트 할 필요없다고 판단함.)
#else
...
#ifdef SLEEP_MODE_MODULE
		app_easy_gap_undirected_advertise_with_timeout_start(user_default_hnd_conf.advertise_period, extend_sleepmode_on);
#else
 ...
 ****************************************************************************************
 */
void JAEM_function_on(void)
{
	systick_timer_used =	app_easy_timer(APP_SYSTICK_TIMER_DELAY, jaem_adc_init);
	led_on();
